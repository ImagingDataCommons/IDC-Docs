...
* [IDC REST API](../api/README.md) and [AI assistant access](../agents/README.md): language-independent access to the same structured cohort filters as the Portal, plus a guarded SQL surface for relational and aggregate queries
...
As we will see in [Organization of data](organization-of-data/files-and-metadata.md), the UUID of a (version of an) instance, and the UUID of the (version of a) series to which it belongs, are used in forming the object (file) name of the corresponding GCS and AWS objects. In addition, each instance version has a corresponding GA4GH DRS object, identified by a GUID based on the instance version's UUID. Refer to the [GA4GH DRS Objects](organization-of-data/guids-and-uuids.md) section for details.
